...
class PokerSequenceDataset(Dataset):
    def __init__(self, hdf5_path, max_seq_len=None):
        ...
        self.max_seq_len = max_seq_len if max_seq_len is not None else self._get_max_seq_len()
    ...
    def __getitem__(self, idx):
        ...
        if seq_len > self.max_seq_len:
            ...
            if pad_length > 0:
                ...
                strategies = np.pad(strategies, (pad_length, 0), 'constant', constant_values=-1)  
                bluffing_probabilities = np.pad(bluffing_probabilities, (pad_length, 0), 'constant', constant_values=-1) 
                mask = np.concatenate((np.zeros(pad_length, dtype=bool), np.ones(seq_len, dtype=bool)))
            else:
        ...
        return {
            ...
            "recent_actions": torch.tensor(recent_actions, dtype=torch.long),         # [seq_len]
            "strategies": torch.tensor(strategies, dtype=torch.long),                 # [seq_len]
            "bluffing_probabilities": torch.tensor(bluffing_probabilities, dtype=torch.float32),  # [seq_len]
...
def poker_collate_fn(batch):
    ...
    """
    states = torch.stack([item["states"] for item in batch])                        # [batch_size, seq_len, input_dim]
    actions = torch.stack([item["actions"] for item in batch])                      # [batch_size, seq_len]
    player_ids = torch.stack([item["player_ids"] for item in batch])                # [batch_size, seq_len]
    positions = torch.stack([item["positions"] for item in batch])                  # [batch_size, seq_len]
    recent_actions = torch.stack([item["recent_actions"] for item in batch])        # [batch_size, seq_len]
    strategies = torch.stack([item["strategies"] for item in batch])                # [batch_size, seq_len]
    bluffing_probabilities = torch.stack([item["bluffing_probabilities"] for item in batch])  # [batch_size, seq_len]
