...
class PokerTransformerModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, seq_len, num_heads, num_layers, 
                 num_players=6, max_positions=10, num_actions=3, num_strategies=3, dropout=0.1):
        """
    ...
    def forward(self, x, player_ids, positions, recent_actions, strategies, bluffing_probabilities, mask=None):
        """
        ...
        # Combine embeddings
        x = x + player_embeds + position_embeds + action_embeds + strategy_embeds + bluffing_embeds  # [batch_size, seq_len, hidden_dim]
        ...
        x = self.transformer_encoder(x, src_key_padding_mask=src_key_padding_mask)  # [seq_len, batch_size, hidden_dim]
