...
class PokerTransformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)  # [batch_size, hidden_dim] -> [batch_size, seq_len=1, hidden_dim]
        ...
        x = x.mean(dim=0)  # [seq_len=1, batch_size, hidden_dim] -> [batch_size, hidden_dim]

        # Pass through classifier
        x = self.classifier(x)  # [batch_size, hidden_dim] -> [batch_size, output_dim]
        return x
